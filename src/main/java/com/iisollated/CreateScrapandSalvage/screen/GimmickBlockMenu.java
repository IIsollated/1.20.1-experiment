...
public class GimmickBlockMenu extends AbstractContainerMenu {
    protected GimmickBlockMenu(int pContainerId, Inventory inv, FriendlyByteBuf extraData) {
        this(pContainerId, inv, inv.player.level().getBlockEntity
                (extraData.readBlockPos()), new SimpleContainerData(27));
    }

    public final GimmickBlockEntity blockEntity;
    private final Level level;
    private final ContainerData data;

    public GimmickBlockMenu(int pContainerId, Inventory inv, BlockEntity entity, ContainerData data) {
        super(ModMenuTypes.GIMMICK_BLOCK_MENU.get(), pContainerId);
        checkContainerSize(inv, 27);
        blockEntity = ((GimmickBlockEntity) entity);
    ...
    public int getScaledProgress() {
        ...
        return maxProgress != 0 && progress != 0 ? progress * progressBite / maxProgress : 0;
    }
    ...
    public boolean stillValid(Player pPlayer) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()),
                pPlayer, ModBlocks.GIMMICK_BLOCK.get());
    }
